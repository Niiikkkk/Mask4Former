...
class CarlaPreprocessing:
    def __init__(
        self,
        data_dir: str = "/normal_dataset",
        save_dir: str = "/home/nicholas/preprocessed/",
        ...
        dirs_ = os.listdir(self.data_dir)
        for dir in dirs_:
            filepaths = list(self.data_dir.glob(f"{dir}/lidar/raw/*npy"))
            train_filepaths = random.sample(filepaths, int(len(filepaths) * 0.7))
            val_filepaths = random.sample(
                list(set(filepaths) - set(train_filepaths)),
                int(len(filepaths) * 0.1),
            )
            test_filepaths = list(set(filepaths) - set(train_filepaths) - set(val_filepaths))
            self.files["train"].extend(sorted([str(file) for file in train_filepaths]))
            self.files["validation"].extend(sorted([str(file) for file in val_filepaths]))
            self.files["test"].extend(sorted([str(file) for file in test_filepaths]))
    ...
    def process_file(self, filepath, mode):
        sequence, scan = re.search(
            r"/home/nicholas/Desktop/main_UE4/output/(\d+)/lidar/raw/lidar-(\d+)\.npy$", filepath
        ).group(1, 2)
        filebase = {
            "filepath": filepath,
    ...
    def extract_instance_from_file(self, filebase):
        ...
        sequence, scan = re.search(
            r"/home/nicholas/Desktop/main_UE4/output/(\d+)/lidar/raw/lidar-(\d+)\.npy$", filebase["filepath"]
        ).group(1, 2)
        file_instances = []
