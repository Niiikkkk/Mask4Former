...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        self, cost_class: float = 1, cost_mask: float = 1, cost_dice: float = 1, cost_box: float = 1, cost_anomaly: float = 1, cost_contrastive: float = 1, cost_pebal_gambler: float = 1, cost_pebal_energy: float = 1
    ):
        """Creates the matcher
        ...
        self.cost_pebal_gambler = cost_pebal_gambler
        self.cost_pebal_energy = cost_pebal_energy
    ...
    def forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            with autocast(enabled=False):
                ...
                # print(out_mask.shape, tgt_mask.shape)  #torch.Size([100, 94716]) torch.Size([26, 94716]) 
            ...
            if torch.isnan(C).any() or torch.isinf(C).any():
                print(f"Warning: Invalid values in cost matrix. NaN: {torch.isnan(C).any()}, Inf: {torch.isinf(C).any()}")
